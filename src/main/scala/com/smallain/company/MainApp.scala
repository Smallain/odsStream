...
//		}
//
//		//TODO 1.添加确定rowkey的唯一方法,不然每次都要添加手写的rowkey
//		//TODO 2.确认为什么有的时候连接kafka会很慢，达不到设定时间的同步，当然是基于原生的sparkstream，可能次情况与结构化流处理的不一致
//		//TODO 3.要点：因为spark是内存计算所以除了spark rdd外任何引用的外部对象都要序列化，通过添加with Serializable序列化属性可以轻松处理
//		ss1.foreachRDD { rdd =>
//			rdd.foreach { record =>
