...
import com.smallain.utils.jsonparse.company._
...
object StructuredStreaming {
  def main(args: Array[String]): Unit = {
    ...
    val temp = dss.filter(x => CanDoDataType.jsonDataFilter(x)).map(x => AlterTableJsonParse.alterJsonParseNewColumns(x))
