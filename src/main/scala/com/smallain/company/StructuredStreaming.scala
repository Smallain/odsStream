...
object StructuredStreaming {
  ...
  def main(args: Array[String]): Unit = {
    if (args.length > 1) {
      ...
      val comment =
        """
          |please set parameter such as :
          | --zookeeper                   zookeeper location path.
          | --table-pk-config             table primary key config file path.
          | --kafka-bootstrap-servers     kafka server address.
          | --kfktopic                    subscribe kafka topic.
          | --spark-check-point           spark checkpoint location.
        """.stripMargin
